...
#[test]
fn compile_requirements_txt_strips_selected_extra_markers() -> Result<()> {
    let context = uv_test::test_context!("3.12");
...
version = "0.1.0"
dependencies = []
optional-dependencies.format-nongpl = [
    "iniconfig==2.0.0",
]
"#,
    )?;
    package.child("src/package/__init__.py").touch()?;

    let package_url = Url::from_directory_path(package.path()).expect("valid directory URL");
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str(&format!("package[format-nongpl] @ {package_url}"))?;
